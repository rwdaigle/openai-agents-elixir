defmodule OpenAI.Agents.Runner do
  ...
  defp call_model(instructions, state) do
    ...
    case state.stream_producer do
      ...
      producer ->
        ...
        stream_events = adapter.create_stream(request, get_api_config(state))
        |> Enum.to_list()
        ...
        response = Enum.reduce(stream_events, %{output: [], usage: %{}}, fn event, acc ->
          case event do
  ...
  defp execute_function_calls(function_calls, state) do
    ...
    {handoff_calls, tool_calls} = Enum.split_with(function_calls, fn call ->
    ...
    case handoff_calls do
      ...
      [] ->
        ...
        function_call_items = Enum.map(tool_calls, fn call ->
          %{
            type: "function_call",
        ...
        result_items = Enum.map(results, fn {call_id, result} ->
          %{
  ...
  defp handle_streaming_completed_function_calls(response, state, producer) do
    ...
    completed_function_calls = Enum.filter(response.output, fn item ->
    ...
    if length(completed_function_calls) > 0 do
      ...
      tool_calls = Enum.map(completed_function_calls, fn call ->
        %{
          id: call["id"],
          name: call["name"],
      ...
      adapter = get_model_adapter(config)
      follow_up_events = adapter.create_stream(follow_up_request, get_api_config(state))
      |> Enum.to_list()
      ...
      follow_up_response = Enum.reduce(follow_up_events, %{output: [], usage: %{}}, fn event, acc ->
        case event do
          %{type: "response.completed", data: data} ->
            response = data["response"] || data
            %{
